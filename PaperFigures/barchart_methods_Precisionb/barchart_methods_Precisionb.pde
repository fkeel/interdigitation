...
void drawChart(String[] names, float[][] values, float[][] colors, int chartWidth, int chartHeight, int scale) {
  ...
  for (int i = 0; i < names.length; i++) {
    ...
    //main bar
    rect(i*spacing+(spacing*0.1), -values[i][0]*scale, spacing*0.5, values[i][0]*scale); //maybe too much
  ...
  for (int i = 0; i < names.length; i++) {
    ...
    textSize(13);
    fill(255);
    text(nf(values[i][0]/25*100, 1, 2), spacing*i+spacing*0.175-2, -0.86*scale+11);
  }
  textSize(14);
  pushMatrix();
  for (int i = 0; i < names.length; i++) {
