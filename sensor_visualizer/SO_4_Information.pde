  ...
  void drawChange(String targetClassB, String pressure) {
    ...
    stroke(200, 0, 20, 240);
    fill(255);
    for (int i = 0; i < avgValue.length-2; i++) { //loop through all records

      line(i*15+15, -abs((avgValue[i+1])-(avgValue[i])), (i+1)*15+15, -abs((avgValue[i+2])-(avgValue[i+1])));
      strokeWeight(2);
